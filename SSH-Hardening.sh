...
firewall_detect_backend() {
    ...
    if [ "$CLIENT_FAMILY" = 6 ]; then
        FIREWALL_IPTABLES_RULES="$FIREWALL_IPTABLES_V6_RULES"
    else
        FIREWALL_IPTABLES_RULES="$FIREWALL_IPTABLES_V4_RULES"
    fi
...
firewall_rollback_allowed_port() {
    ...
    if [ "${FIREWALL_ALLOW_FIREWALLD_RUNTIME_ADDED:-no}" = no ] \
        && [ "${FIREWALL_ALLOW_FIREWALLD_PERMANENT_ADDED:-no}" = no ]; then
        FIREWALL_ALLOW_FIREWALLD_ADDED=no
    fi
    ...
    if [ "$FIREWALLD_ACTIVE" = true ]; then
        if ! firewall-cmd --permanent --zone="$FIREWALL_ALLOW_FIREWALLD_ZONE" \
            ...
            if firewall-cmd --permanent --zone="$FIREWALL_ALLOW_FIREWALLD_ZONE" \
                ...
                FIREWALL_ALLOW_FIREWALLD_ADDED=yes
                FIREWALL_ALLOW_DIRTY=yes
            else
                FAILED=yes
