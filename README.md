...
查看、添加和删除都会先选择目标用户，并依据 `sshd -T -C` 解析该用户实际生效的 `AuthorizedKeysFile`；只有“生成密钥对后直接加入”仍默认操作 root 的 `/root/.ssh/authorized_keys`。查看功能会标明密钥所在文件及物理行号；删除功能不会按公钥主体批量删除重复项，而是只删除本次选中的那一行。
