...
monitor_alert_cron_status() {
    ...
    case "$KIND" in
        daily)
            crontab -l 2>/dev/null | grep -q 'vps-monitor-daily-report' && echo "已安装" || echo "未安装"
            ;;
        *)
            crontab -l 2>/dev/null | grep -q 'vps-monitor-alert' && echo "已安装" || echo "未安装"
            ;;
    esac
}
...
monitor_alert_resource_check() {
    ...
    if [ -z "${ISSUES// }" ]; then
        if [ "$PREV_STATE" = alert ] && [ "${MON_RECOVERY_ENABLED:-yes}" = yes ] && ! monitor_alert_in_silence; then
            monitor_alert_notify "✅ <b>VPS 监控恢复</b>" "主机：<code>${HOST}</code>\n时间：<code>$(date '+%Y-%m-%d %H:%M:%S')</code>\n状态：<code>资源告警已恢复</code>"
