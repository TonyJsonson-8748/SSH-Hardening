...
fail2ban_menu() {
    while true; do
        ...
        box_line "  服务: ${F2B_ST}  jail: ${JAIL_NAME}" \
                 "  服务: ${F2B_COLOR}${BOLD}${F2B_ST}${NC}  jail: ${BOLD}${JAIL_NAME}${NC}"
        box_line "  封禁IP: ${BANNED_COUNT}  总失败: ${TOTAL_FAIL}  监控端口: ${CUR_PORT}" \
                 "  封禁IP: ${RED}${BOLD}${BANNED_COUNT}${NC}  总失败: ${YELLOW}${BOLD}${TOTAL_FAIL}${NC}  端口: ${BOLD}${CUR_PORT}${NC}"
        box_line "  封禁时长: ${BAN_HUMAN}  窗口: ${FIND_HUMAN}  最大重试: ${CUR_MAX}次" \
                 "  封禁时长: ${BOLD}${BAN_HUMAN}${NC}  窗口: ${BOLD}${FIND_HUMAN}${NC}  最大重试: ${BOLD}${CUR_MAX}${NC}次"
        box_sep
...
f2b_logs() {
    ...
    if [ ! -f "$LOG_FILE" ]; then
        ...
        journalctl -u fail2ban -n 30 --no-pager 2>/dev/null \
            | grep -E "Ban|Unban|Found|WARNING|ERROR" \
            | while IFS= read -r line; do
...
bbr_menu_manual() {
    ...
    bbr_confirm_apply "$RMEM" "$WMEM" "$TCP_MEM" "$NOTSENT" "$ADV_WIN" \
        "$MIN_FREE" "$SWAP" "$TCP_RMEM_DEFAULT" \
        "手动选择（内存 ${MEM_MB}MB）" "$BUF_LBL"
}

is_openvz() {
    [ -f /proc/user_beancounters ] || \
    grep -qa "container=openvz" /proc/1/environ 2>/dev/null || \
    [ -d /proc/vz ]
}
