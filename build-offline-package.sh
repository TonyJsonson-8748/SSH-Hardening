...
SOURCE_SCRIPT="${SOURCE_SCRIPT:-$ROOT/SSH-Hardening.sh}"
...
/usr/local/bin/vps-tools plus non-conflicting v/V shortcuts.
EOF
}
...
TMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/vps-tools-offline.XXXXXX")
trap 'rm -rf "$TMP_DIR"' EXIT HUP INT TERM
PACKAGE_DIR="$TMP_DIR/vps-tools-offline-$VERSION"
mkdir -p "$PACKAGE_DIR" "$OUTPUT_DIR"
cp "$SOURCE_SCRIPT" "$PACKAGE_DIR/SSH-Hardening.sh"
ACTUAL_HASH=$(file_sha256 "$PACKAGE_DIR/SSH-Hardening.sh" || true)
[ -n "$ACTUAL_HASH" ] || fail "no SHA256 utility is available"
printf '%s  %s\n' "$ACTUAL_HASH" "SSH-Hardening.sh" > "$PACKAGE_DIR/SSH-Hardening.sh.sha256"
...
SCRIPT="$PACKAGE_DIR/SSH-Hardening.sh"
CHECKSUM="$PACKAGE_DIR/SSH-Hardening.sh.sha256"
...
TARGET="$BIN_DIR/vps-tools"
mkdir -p "$BIN_DIR" || fail "cannot create $BIN_DIR"
TEMP="$BIN_DIR/.vps-tools.install.$$"
cp "$SCRIPT" "$TEMP" || fail "cannot write $TEMP"
chmod 755 "$TEMP"
mv -f "$TEMP" "$TARGET" || fail "cannot install $TARGET"
...
printf 'Installed VPS Tools to %s\n' "$TARGET"
printf 'SHA256 verified: %.16s...\n' "$ACTUAL"
INSTALLER
chmod 755 "$PACKAGE_DIR/install.sh"

cat > "$PACKAGE_DIR/README.txt" <<EOF
VPS Tools offline package ($VERSION)
...
2. Verify the archive: sha256sum -c $(basename "vps-tools-offline-$VERSION.tar.gz.sha256")
3. Extract it: tar -xzf vps-tools-offline-$VERSION.tar.gz
4. Install it: cd vps-tools-offline-$VERSION && sudo bash install.sh

The installer verifies the bundled script before placing it in /usr/local/bin/vps-tools.
...
ARCHIVE="$OUTPUT_DIR/vps-tools-offline-$VERSION.tar.gz"
tar -czf "$ARCHIVE" -C "$TMP_DIR" "vps-tools-offline-$VERSION"
